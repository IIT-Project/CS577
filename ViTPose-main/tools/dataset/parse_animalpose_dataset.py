...
def xml2coco_trainval(file_list, img_root, save_path, start_ann_id=0):
    ...
    for file in file_list:
        ...
        if img_id not in img_ids:
            ...
            # join the path of image root and image name
            image_name = data_anno['image'] + '.jpg'
...
def split_train_val_test(work_dir, trainval_file, train_file, val_file,test_file,
                    val_ann_num,test_ann_num):
    """Split train-val json file into training and validation files.
    ...
    for img_id in img_list:
        ...
        if count + len(ann_ids) <= val_ann_num:
            ...
            for ann_id in ann_ids:
                annotations_val.append(coco.anns[ann_id])
        elif count + len(ann_ids) <= val_ann_num+test_ann_num:
            # for test
            count += len(ann_ids)
            images_test.append(coco.imgs[img_id])
            for ann_id in ann_ids:
                annotations_test.append(coco.anns[ann_id])
        else:
            images_train.append(coco.imgs[img_id])
            for ann_id in ann_ids:
                annotations_train.append(coco.anns[ann_id])
    ...
    cocotype_test['info'][
        'description'] = 'AnimalPose dataset Generated by MMPose Team'
    cocotype_test['info']['version'] = '1.0'
    cocotype_test['info']['year'] = time.strftime('%Y', time.localtime())
    cocotype_test['info']['date_created'] = time.strftime(
        '%Y/%m/%d', time.localtime())
    cocotype_test['images'] = images_test
    cocotype_test['annotations'] = annotations_test
    cocotype_test['categories'] = category_info

    json.dump(
        cocotype_test, open(os.path.join(work_dir, test_file), 'w'), indent=4)
    print('=========================================================')
    print('number of images:', len(images_test))
    print('number of annotations:', len(annotations_test))
    print(f'done {test_file}')


if __name__ == '__main__':
    dataset_dir = r'D:\Development\test'
    img_dir=r'D:\Development\test\VOC2012\JPEGImages'
    ...
    split_train_val_test(os.path.join(dataset_dir, 'annotations'),
