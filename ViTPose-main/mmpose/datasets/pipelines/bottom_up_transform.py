...
class BottomUpRandomAffine:
    ...
    def __call__(self, results):
        ...
        assert isinstance(joints, list)
        assert len(mask) == len(joints)
        assert len(mask) == len(self.output_size), (len(mask),len(self.output_size),self.output_size)
...
class BottomUpGetImgSize:
    ...
    def __call__(self, results):
        ...
        if input_size.size > 1:
            ...
            input_size = np.array([input_size, input_size], dtype=np.int64)
        img = results['img']
    ...
    def __call__(self, results):
        ...
        for _, s in enumerate(sorted(test_scale_factor, reverse=True)):
            ...
            image_resized = mmcv.imresize(
                _results['img'], input_size, interpolation='bicubic')
            _results['img'] = image_resized
